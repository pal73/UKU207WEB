	...
	case 'd':
		...
		switch (env[1]) {
			case '0':
				...
				switch (env[2]) {
					case '1':
						//len = sprintf((char *)buf,(const char *)&env[4],"АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЬЫЪЭЮЯ1?°абвгдеёжзийклмнопрстуфхцчшщьыъэюя");
						len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("АБВГД"));
					break;
			        case '2':
